...
def print_docs(doc_ids, df):
    print(df[df.index.isin(doc_ids)].titles.to_markdown())
...
def search(title_query: str, abstract_query: str, max_result_count: int = 10, method: str = 'ltn-lnn',
    ...
    for writer in writers:
        if method == 'okapi25':
            result = compute_okapi(t_q_tokens, a_q_tokens, weight, doc_dict=doc_dict[writer], okapi_idf=okapi_idf[writer], tf_doc=tf_doc[writer])
        else:
            result = compute_similarity(t_q_tokens, a_q_tokens, weight, method[-1], tf_doc=tf_doc[writer], idf_dict=idf_dict[writer])
        final_results = merge_lists(final_results, result)

    final_results.sort(key=lambda tup: tup[1], reverse=True)
    doc_ids = final_results[:max_result_count]
